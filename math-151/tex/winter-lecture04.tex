...
\begin{definition}\label{defn:mapping-cylinder}
Let $X$ and $Y$ be topological spaces. Let $f\colon X\to Y$ be a continuous map.
We define the \define{Mapping Cylinder} $\MappingCylinder{f}$ to be the quotient space
\begin{equation}
\MappingCylinder{f}=((X\times[0,1])\sqcup Y)/(\forall x\in X\ldotp(x,1)\sim f(x)).
\end{equation}
Then we identify $X\subset\MappingCylinder{f}$ as $X\times\{0\}$; we also have
$Y\subset\MappingCylinder{f}$. Furthermore, $\MappingCylinder{f}$ deformation retracts to $Y$ (by
``crushing the cylinder'').
\end{definition}

\begin{remark}
We discussed the mapping cylinder the first lecture of Fall Quarter,
but did not make extensive use of the notion.
\end{remark}
...
In general, we use the notion of a mapping cylinder $\MappingCylinder{f}$. We will
show $X\homotopic\MappingCylinder{f}\homotopic Y$. In particular, $Y\homotopic\MappingCylinder{f}$
by deformation retract.

So it suffices to show $X\homotopic\MappingCylinder{f}$. We approximate continuous
$f$ by a CW map, so without loss of generality $f$ is a CW map sending
cells to cells. Then it is easy to see $X\times\{0\}\subset\MappingCylinder{f}$ is
...
instead of $f$. So we have $X\homotopic\MappingCylinder{f'}\homotopic Y$.
\end{proof}
