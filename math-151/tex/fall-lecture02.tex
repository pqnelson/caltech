...
\begin{example}
Consider $X=\sphere{1}$ the circle, and $X^{0}=\point{x}$. Then
$X^{1}=\sphere{1}$ is obtained by mapping the boundary of $\disk{1}$ to $X^{0}$.
\end{example}

\begin{example}
Let $X=\sphere{n}$ be the $n$-sphere. We have $X^{0}=\point{x}$, and
$X^{k}=X^{0}$ for all $k<n$. Then $X^{n}=\sphere{n}$ using $f\colon\boundary\disk{n}\to\point{x}$.
...
\begin{example}
We can pick for $\sphere{1}$ the CW complex where $X^{0}$ consists of two
...
This construction can be iterated by treating $\sphere{1}$ as the equator,
...
$\sphere{n-1}$ to obtain the CW complex for $\sphere{n}$. We see that
$\sphere{0}\subset\sphere{1}\subset\sphere{2}\subset\dots$. Then we could define
\begin{equation}
\sphere{\infty} := \bigcup^{\infty}_{n=0}\sphere{n}.
\end{equation}
...
\begin{example}[Real projective spaces]\label{ex:fall-lec02:real-projective-spaces}
Now, we see that 
\begin{equation}
\sphere{n}/(\mbox{antipodal map})=\RP^{n}.
\end{equation}
We thus get a CW complex describing $\RP^{n}$, with one $k$-cell for
each $k=0,1,\dots,n$. This also gives us a way to define
\begin{equation}
\RP^{\infty}:=\sphere{\infty}/(\mbox{antipodal map}).
\end{equation}
\end{example}

\begin{example}[Complex projective spaces]\label{ex:fall-lec02:complex-projective-spaces}%
Consider
\begin{equation}
...
\begin{example}
$\sphere{1}\subset \sphere{2}\subset\dots\subset \sphere{\infty}$ gives us many subcomplexes.
\end{example}
