...
\begin{equation}
\MappingCylinder{f}=(Z\times I\sqcup X)/(z,1)\sim f(z).
\end{equation}
Define $W:=\MappingCylinder{f}/(a,t)\sim(a,0)$ for all $a\in A$, where we are
...
\begin{observation}
The punchline is: it is easier to use the mapping cylinder (\S\ref{defn:mapping-cylinder}) than a map.
\end{observation}
...
Consider the mapping cylinder (\S\ref{defn:mapping-cylinder}) $\MappingCylinder{f}$ of $f$, which gives us the
following commutative diagram
\begin{equation}
\vcenter{\xymatrix{& \MappingCylinder{f}\ar[rd]^{r} &\\
X \ar@{^{(}->}[ur]^{j}\ar[rr]^{f} & & Y}}
\end{equation}
It suffices to prove the inclusion $j\colon X\into\MappingCylinder{f}$ satisfies
...
\begin{proposition}\label{prop:existence-of-postnikov-tower}
...
\end{equation}
This vertical tower of $X_{n}$ is precisely the \define{Postnikov Tower}.
\end{proof}